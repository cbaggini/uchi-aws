...
const Switch = ({
  ...
  const handleCheckbox = (e, position) => {
    ...
    if (!e.target.checked) {
      const unchecked = tempArray.filter(
        (artWork) => artWork.content_type !== mediaTypes[position]
      );
      ...
      const checked = tempArray.filter(
        (artWork) => artWork.content_type === mediaTypes[position]
      );
        ...
        return (
          <div key={index} className={mediaType}>
            <div>
              ...
              />
              <label className="switch-label" htmlFor={index}>
